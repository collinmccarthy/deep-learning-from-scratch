# From GitHub:  huggingface/timm/models/vision_transformer.py
...
class ParallelScalingBlock(nn.Module):
    ...
    def __init__(
        ...
        self.ls = (
            LayerScale(dim, init_values=init_values) if init_values is not None else nn.Identity()
        )
...
class VisionTransformer(nn.Module):
    ...
    def __init__(
        ...
        reduction = (
            self.patch_embed.feat_ratio() if hasattr(self.patch_embed, "feat_ratio") else patch_size
        )
...
def checkpoint_filter_fn(
    ...
    if "visual.class_embedding" in state_dict:
        ...
        "visual.trunk.pos_embed" in state_dict or "visual.trunk.blocks.0.norm1.weight" in state_dict
    ):
...
_quick_gelu_cfgs = [
    n for n, c in default_cfgs.items() if c.get("notes", ()) and "quickgelu" in c["notes"][0]
]
...
def vit_base_patch16_plus_clip_240(pretrained: bool = False, **kwargs) -> VisionTransformer:
    ...
    model_args = dict(
        patch_size=16, embed_dim=896, depth=12, num_heads=14, pre_norm=True, norm_layer=nn.LayerNorm
    )
    model = _create_vision_transformer(
...
def test_vit(pretrained: bool = False, **kwargs) -> VisionTransformer:
    """ViT Test"""
    model_args = dict(
        patch_size=16, embed_dim=64, depth=6, num_heads=2, mlp_ratio=3, dynamic_img_size=True
    )
    model = _create_vision_transformer(
