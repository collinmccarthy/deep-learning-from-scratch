# From GitHub:  huggingface/timm/models/convnext.py
...
from timm.layers import (
    ...
    AvgPool2dSame, # pyright: ignore[reportPrivateImportUsage]
    DropPath, # pyright: ignore[reportPrivateImportUsage]
    Mlp, # pyright: ignore[reportPrivateImportUsage]
    GlobalResponseNormMlp, # pyright: ignore[reportPrivateImportUsage]
    LayerNorm2d, # pyright: ignore[reportPrivateImportUsage]
    LayerNorm, # pyright: ignore[reportPrivateImportUsage]
    RmsNorm2d, # pyright: ignore[reportPrivateImportUsage]
    RmsNorm, # pyright: ignore[reportPrivateImportUsage]
    create_conv2d, # pyright: ignore[reportPrivateImportUsage]
    get_act_layer, # pyright: ignore[reportPrivateImportUsage]
    get_norm_layer, # pyright: ignore[reportPrivateImportUsage]
    make_divisible, # pyright: ignore[reportPrivateImportUsage]
    to_ntuple, # pyright: ignore[reportPrivateImportUsage]
)
from timm.layers import NormMlpClassifierHead, ClassifierHead  # pyright: ignore[reportPrivateImportUsage]
...
class ConvNeXtBlock(nn.Module):
    ...
    def __init__(
        ...
        self.gamma = (
            nn.Parameter(ls_init_value * torch.ones(out_chs)) if ls_init_value is not None else None
        )
    ...
    def __init__(
        ...
        if stem_type == "patch":
            ...
            self.stem = nn.Sequential(
                nn.Conv2d(
                    in_chans, dims[0], kernel_size=patch_size, stride=patch_size, bias=conv_bias
                ),
                norm_layer(dims[0]),
            )
            ...
            self.stem = nn.Sequential(
                *filter(
                    ...
                    [
                        nn.Conv2d(
                            in_chans, mid_chs, kernel_size=3, stride=2, padding=1, bias=conv_bias
                        ),
...
def convnext_zepto_rms(pretrained=False, **kwargs) -> ConvNeXt:
    ...
    model_args = dict(
        depths=(2, 2, 4, 2), dims=(32, 64, 128, 256), conv_mlp=True, norm_layer="rmsnorm2d"
    )
...
def convnext_atto_ols(pretrained=False, **kwargs) -> ConvNeXt:
    ...
    model_args = dict(
        depths=(2, 2, 6, 2), dims=(40, 80, 160, 320), conv_mlp=True, stem_type="overlap_tiered"
    )
    model = _create_convnext(
...
def convnext_atto_rms(pretrained=False, **kwargs) -> ConvNeXt:
    # timm femto variant (NOTE: still tweaking depths, will vary between 3-4M param, current is 3.7M
    model_args = dict(
        depths=(2, 2, 6, 2), dims=(40, 80, 160, 320), conv_mlp=True, norm_layer="rmsnorm2d"
    )
    model = _create_convnext(
...
def convnext_femto_ols(pretrained=False, **kwargs) -> ConvNeXt:
    # timm femto variant
    model_args = dict(
        depths=(2, 2, 6, 2), dims=(48, 96, 192, 384), conv_mlp=True, stem_type="overlap_tiered"
    )
    model = _create_convnext(
...
def convnext_pico_ols(pretrained=False, **kwargs) -> ConvNeXt:
    ...
    model_args = dict(
        depths=(2, 2, 6, 2), dims=(64, 128, 256, 512), conv_mlp=True, stem_type="overlap_tiered"
    )
    model = _create_convnext(
...
def convnext_nano_ols(pretrained=False, **kwargs) -> ConvNeXt:
    ...
    model_args = dict(
        depths=(2, 2, 8, 2), dims=(80, 160, 320, 640), conv_mlp=True, stem_type="overlap"
    )
    model = _create_convnext(
...
def convnext_tiny_hnf(pretrained=False, **kwargs) -> ConvNeXt:
    ...
    model_args = dict(
        depths=(3, 3, 9, 3), dims=(96, 192, 384, 768), head_norm_first=True, conv_mlp=True
    )
    model = _create_convnext(
...
def convnext_xxlarge(pretrained=False, **kwargs) -> ConvNeXt:
    model_args = dict(
        depths=[3, 4, 30, 3], dims=[384, 768, 1536, 3072], norm_eps=kwargs.pop("norm_eps", 1e-5)
    )
    model = _create_convnext(
...
def convnextv2_base(pretrained=False, **kwargs) -> ConvNeXt:
    model_args = dict(
        depths=[3, 3, 27, 3], dims=[128, 256, 512, 1024], use_grn=True, ls_init_value=None
    )
...
def convnextv2_large(pretrained=False, **kwargs) -> ConvNeXt:
    model_args = dict(
        depths=[3, 3, 27, 3], dims=[192, 384, 768, 1536], use_grn=True, ls_init_value=None
    )
    model = _create_convnext(
...
def convnextv2_huge(pretrained=False, **kwargs) -> ConvNeXt:
    model_args = dict(
        depths=[3, 3, 27, 3], dims=[352, 704, 1408, 2816], use_grn=True, ls_init_value=None
    )
