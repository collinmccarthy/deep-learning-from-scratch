...
class Attention(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        N, L, C = x.shape
        # (N,L,C) -> (N,L,3C) -> (N,L,3,H,D) -> (3,N,H,L,D)
        ...
        if self.fused_attn:
            ...
            q = q * self.scale
            attn = q @ k.transpose(-2, -1)  # N,H,L,D @ N,H,D,L -> N,H,L,L
            attn = attn.softmax(dim=-1)
            attn = self.attn_drop(attn)
            x = attn @ v  # N,H,L,L @ N,H,L,D -> N,H,L,D

        x = x.transpose(1, 2).reshape(N, L, C)  # N,H,L,D -> N,L,H,D -> N,L,C
        x = self.proj(x)
        x = self.proj_drop(x)
        return x
...
class CausalAttention(nn.Module):
    ...
    def __init__(
        ...
        self.tril = torch.tril(torch.ones_like((dim)))
